...
def download_ytb_mp3(song_title, song_author, song_id):
    ...
    try:
        ydl_opts = {
            ...
            'no_warnings': True,
            'postprocessors': [],
            'cookiefile': '/usr/src/app/cookies.txt',  # Path to your cookies file, to bypass bot detection
        }
        ...
        ffmpeg.input(downloaded_file).output(mp3_file, acodec='libmp3lame', audio_bitrate='64k').run(overwrite_output=True)
